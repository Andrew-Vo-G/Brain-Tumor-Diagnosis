(async function () {
  const me = await AppCommon.initPage({ requireRole: "doctor", allowGuest: true });
  if (!me) return;
  ...
  if (me.role === "guest") {
    ...
    weeklyCasesEl.textContent = "0";
    analyzedCasesEl.textContent = "0";
    tableBody.innerHTML =
      '<tr><td colspan="6" class="px-6 py-8 text-center text-slate-500">Please sign in as a doctor to load data and use these features.</td></tr>';
    setStatus("Guest mode: preview only.");
    return;
  }
  ...
  try {
    ...
    const [patients, records] = await Promise.all([API.getPatients(), API.getRecords()]);
    ...
    if (!patients.length) {
      tableBody.innerHTML =
        '<tr><td colspan="6" class="px-6 py-8 text-center text-slate-500">No patients yet.</td></tr>';
    ...
    tableBody.innerHTML =
      '<tr><td colspan="6" class="px-6 py-8 text-center text-red-600">Failed to load dashboard data.</td></tr>';
